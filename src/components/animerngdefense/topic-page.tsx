...
import type { TopicPage as TopicPageData } from '@/data/animerngdefense/types';
      ...
      <Container className="space-y-8 px-4">
        <header className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
          <div className="max-w-3xl space-y-4">
            ...
            <p className="text-[#E6D7EC] text-lg leading-8">{topic.summary}</p>
            <LastUpdated date={topic.updatedAt} />
          </div>
          <div className="relative aspect-video overflow-hidden rounded-lg border border-[#4A254F] bg-[#120915]">
            <Image
              src={topic.image}
          ...
          {topic.featuredRoutes.map((route) => (
            ...
            >
              <div className="flex items-center justify-between gap-3">
                ...
                <ArrowRight className="size-4 shrink-0 text-[#6EE7B7] transition group-hover:translate-x-0.5" />
              </div>
        ...
        <article className="rounded-lg border border-[#4A254F] bg-[#120915] p-6 md:p-8">
          ...
          <div className="mt-10 flex flex-wrap gap-3">
            ...
            <Button
              asChild
              variant="outline"
              className="border-[#6EE7B7] bg-[#09060F] text-[#FFE7A8] hover:bg-[#6EE7B7] hover:text-[#042013]"
            >
