...
import { relatedRouteLabels } from '@/data/icetycoon2/guides';
import { officialGameFacts } from '@/data/icetycoon2/sources';
import type { TopicPage as TopicPageData } from '@/data/icetycoon2/types';
      ...
      <Container className="space-y-8 px-4">
        <header className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
          <div className="max-w-3xl space-y-4">
            ...
            <p className="text-[#B8CEDB] text-lg leading-8">{topic.summary}</p>
            <LastUpdated date={topic.updatedAt} />
          </div>
          <div className="relative aspect-video overflow-hidden rounded-lg border border-[#263B4D] bg-[#0D1A2B]">
            <Image
              src={topic.image}
          ...
          {topic.featuredRoutes.map((route) => (
            ...
            >
              <div className="flex items-center justify-between gap-3">
                <h2 className="font-display text-xl font-bold text-[#FFE7B8]">
        ...
        <article className="rounded-lg border border-[#263B4D] bg-[#0D1A2B] p-6 md:p-8">
          ...
          <div className="mt-10 flex flex-wrap gap-3">
            ...
            <Button
              asChild
              variant="outline"
              className="border-[#FFB347] bg-[#07111F] text-[#FFE7B8] hover:bg-[#FFB347] hover:text-[#07111F]"
            >
